                ...
                if (curState.type === 'rest') {
                    fetch(sdsource + curState.url).then(function (res) {
                        res.json().then(function (data) {
                            ...
                            // If there is no data
                            if (!data) {
                                console.warn('Function >' + curSet.name + '< is not available in this installation. Check the tbl_systemconfiguration entry exists, if you think this is an error.');
...
adm_functions.onDeActivate = function (evt) {
    ...
    Promise.all(colProms).then(function () {
        ...
        // Check if table changes are needed
        if(!def) {
...
//            location.reload();
            ...
            if (curAct.type === 'rest') {
                ...
                fetch(sdsource + curAct.url, {
                    ...
                    if (res.ok) {
                        ...
                        res.json().then(function (data) {
                            UIkit.modal.alert(window.swac.lang.dict.app.adm_funtionactivaterr + ' ' + data);
                        });
                    }
                }).catch(function (err) {
                    console.error('Error: ' + err);
        ...
        UIkit.modal.alert(window.swac.lang.dict.app.adm_funtiontblinstallerr + err);
    });
};